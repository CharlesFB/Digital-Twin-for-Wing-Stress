...
{
    public class LineGraphTest1 : MonoBehaviour
    {
        ...
        public void EnqueueQL(float x, int i)
        {
            switch (i)
            {
                case 1:
        ...
        float GenerateMockedLine(float x)
        {
            ...
            float[] ml = SliceFloatArray(mocklist, imock, 100);
        ...
        private float GenerateFromQueue(float x, int i)
        {
            switch (i)
            {
                ...
                case 2:
                    int dx2 = (int)Mathf.Floor(x);
                    if (ql3 == null || ql3.Count == 0) return 0;
                    if (dx2 < 0 || dx2 >= 100) return 0;

                    int iq2 = 0;

                    foreach (float q in ql3)
                    {
                        if (iq2 == dx2)
                            return q;
                        iq2 += 1;
        ...
        {
            //if (sc.floatArrays == null || sc.floatArrays[0].Count == 0) return;
            //List<float> fl = sc.floatArrays[0];
            //li = new Queue<float>();
            //for (int i = 0; i < fl.Count; i += skip)
            //{
            //    li.Enqueue(fl[i]);
            //}
        }
